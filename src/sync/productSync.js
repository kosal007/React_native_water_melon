    ...
    try {
      await database.write(async () => {
        ...
        await productsCollection().create((product) => {
          ...
          product.updatedAt = Number(timestamp);
...
function buildPushPayload(changes, timestamp) {
  ...
  const created = rawCreated
    ...
    .filter((record) => Boolean(record.name) && Number.isFinite(record.price));
...
async function touchProductRecords(changes, timestamp) {
  ...
  await database.write(async () => {
    await Promise.all(
      rows.map((row) =>
        row.update((record) => {
          record.updatedAt = Number(timestamp);
        }),
      ),
    );
  });
